...
//            height = view2.getMeasuredHeight();
    ...
    public int getItemCount() {
        int size = mList.size();
        if (0 == size) {
            return 0 + mHeaderViewInfos.size();
        } else {
            if (isLoadmore) {
                return size + 1 + mHeaderViewInfos.size();
            }
            return size + mHeaderViewInfos.size();
        }
    }
    ...
    public void notifyDatasChanged(final List<T> data, boolean isLoadingmore) {
        ...
        if (data == null || data.size() < UserInfoConfig.ROWS) {//数据小于10就说明没有更多数据了
            onNoDatas();
        }
