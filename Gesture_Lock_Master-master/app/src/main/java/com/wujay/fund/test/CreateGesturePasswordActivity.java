...
import com.wujay.fund.view.ModleView;

import java.util.ArrayList;
import java.util.List;

public class CreateGesturePasswordActivity extends AppCompatActivity implements
    ...
    protected List<LockPatternView.Cell> mChosenPattern = null;//保存的绘制轨迹
...
//            }
    ...
    private Runnable mClearPatternRunnable = new Runnable() {
        public void run() {
            mLockPatternView.clearPattern();//清空模式
        }
    };
    ...
    protected LockPatternView.OnPatternListener mChooseNewLockPatternListener = new LockPatternView.OnPatternListener() {
        ...
        public void onPatternDetected(List<LockPatternView.Cell> pattern) {
            ...
            if (mUiStage == Stage.NeedToConfirm
                    || mUiStage == Stage.ConfirmWrong) {//再次绘制  或者 与上次输入不一致，请重试
                if (mChosenPattern == null)
                    throw new IllegalStateException(
                            "null chosen pattern in stage 'need to confirm");
                if (mChosenPattern.equals(pattern)) {//2次一致保存
...
//                    saveChosenPatternAndFinish();//TODO 自动保存密码，关闭界面
                    ...
                    || mUiStage == Stage.ChoiceTooShort) {//初次绘制 或者 至少需要连接4个点，请重新绘制
                if (pattern.size() < LockPatternUtils.MIN_LOCK_PATTERN_SIZE) {//至少需要连接4个点，请重新绘制
                    updateStage(Stage.ChoiceTooShort);
                } else {
                    mChosenPattern = new ArrayList<LockPatternView.Cell>(
                            pattern);
                    updateStage(Stage.FirstChoiceValid);//图案已记录
                    updateStage(Stage.NeedToConfirm);//请再次绘制解锁图案 TODO 自动进入下一步
                }
            } else {
