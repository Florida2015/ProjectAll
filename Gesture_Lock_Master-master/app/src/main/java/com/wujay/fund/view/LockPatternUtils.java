...
public class LockPatternUtils {
    ...
    private static class LockPatternFileObserver extends FileObserver {
        ...
        public void onEvent(int event, String path) {
            ...
            if (("gesture.key_" ).equals(path)) {//+ SPUtils.geTinstance().getUid()
                Log.d(TAG, "lock pattern file changed");//+ SPUtils.geTinstance().getUid()
                sHaveNonZeroPatternFile.set(new File(dataSystemDirectory, ("gesture.key_" )).length() > 0);
            }
        }
    }
