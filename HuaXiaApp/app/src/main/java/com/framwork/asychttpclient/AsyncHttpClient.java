...
public class AsyncHttpClient {
    ...
    private static SchemeRegistry getDefaultSchemeRegistry(boolean fixNoHttpResponseException, int httpPort, int httpsPort) {
        if (fixNoHttpResponseException) {
            MyLog.d(LOG_TAG, "Beware! Using the fix is insecure, as it doesn't verify SSL certificates.");
        }

        if (httpPort < 1) {
            httpPort = 80;
            MyLog.d(LOG_TAG, "Invalid HTTP port number specified, defaulting to 80");
        }

        if (httpsPort < 1) {
            httpsPort = 443;
            MyLog.d(LOG_TAG, "Invalid HTTPS port number specified, defaulting to 443");
        }
    ...
    private HttpEntity paramsToEntity(RequestParams params, ResponseHandlerInterface responseHandler) {
        ...
        try {
            ...
            if (responseHandler != null)
                responseHandler.sendFailureMessage("",0, null, null, t);//sj 这里的地址并不好获取,暂不获取
            else
                t.printStackTrace();
        }
