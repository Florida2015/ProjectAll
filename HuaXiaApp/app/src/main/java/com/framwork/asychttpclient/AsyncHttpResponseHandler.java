...
package com.framwork.asychttpclient;
...
public abstract class AsyncHttpResponseHandler implements ResponseHandlerInterface {
     ...
     */
    public abstract void onSuccess(String reqUrl,int statusCode, Header[] headers, byte[] responseBody);
     ...
     */
    public abstract void onFailure(String reqUrl,int statusCode, Header[] headers, byte[] responseBody, Throwable error);
    ...
    public void onRetry(int retryNo) {
        MyLog.d(LOG_TAG, String.format("Request retry no. %d", retryNo));
    }

    public void onCancel() {
        MyLog.d(LOG_TAG, "Request got cancelled");
    }
    ...
    final public void sendSuccessMessage(String reqString, int statusCode, Header[] headers, byte[] responseBytes) {
        sendMessage(obtainMessage(SUCCESS_MESSAGE, new Object[]{reqString, statusCode, headers, responseBytes}));
    }

    final public void sendFailureMessage(String reqString,int statusCode, Header[] headers, byte[] responseBody, Throwable throwable) {
        sendMessage(obtainMessage(FAILURE_MESSAGE, new Object[]{reqString,statusCode, headers, responseBody, throwable}));
    }
        ...
        switch (message.what) {
            case SUCCESS_MESSAGE:
                ...
                if (response != null && response.length >= 3) {
                    onSuccess((String) response[0],(Integer) response[1], (Header[]) response[2], (byte[]) response[3]);
                } else {
            ...
            case FAILURE_MESSAGE:
                ...
                if (response != null && response.length >= 4) {
                    onFailure((String) response[0],(Integer) response[1], (Header[]) response[2], (byte[]) response[3], (Throwable) response[4]);
                } else {
    ...
    @Override
    public void sendResponseMessage(String reqUrl,HttpResponse response) throws IOException {
        ...
        if (!Thread.currentThread().isInterrupted()) {
            ...
            if (!Thread.currentThread().isInterrupted()) {
                if (status.getStatusCode() >= 300) {
                    sendFailureMessage(reqUrl,status.getStatusCode(), response.getAllHeaders(), responseBody, new HttpResponseException(status.getStatusCode(), status.getReasonPhrase()));
                } else {
                    sendSuccessMessage(reqUrl,status.getStatusCode(), response.getAllHeaders(), responseBody);
                }
            }
        }
    }
