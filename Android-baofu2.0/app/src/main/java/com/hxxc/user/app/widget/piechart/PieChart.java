package com.hxxc.user.app.widget.piechart;
...
public class PieChart extends View {
    ...
    //Touch
    private boolean touchFlag = false;//true
    //百分比文字
    private boolean displayFlag = false;//true

    private float[] pieAngles;
    private int angleId;
    private double offsetScaleRadius = 1.1;//1.1
    //圆环半径比例
    private double widthScaleRadius = 0.8;//弧度的宽度 0.8
    private double radiusScaleTransparent = 0.95;//0.5
    private double radiusScaleInside = 0.92;//0.43
    //Paint的字体大小
...
//        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mPieData == null)
            return;
        float currentStartAngle = -90;//0 当前起始角度
        ...
        for (int i=0; i<n; i++){
            switch (i){
                ...
                case R.styleable.PieChart_percentDecimal:
                    ...
                    break;
                case R.styleable.PieChart_textSize_p:
