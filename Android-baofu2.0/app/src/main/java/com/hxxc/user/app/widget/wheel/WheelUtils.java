package com.hxxc.user.app.widget.wheel;
...
import com.hxxc.user.app.widget.WheelViewDialog;
...
public class WheelUtils implements OnWheelChangedListener, OnClickListener {

	private Activity mActivity;

	public WheelUtils(Activity activity) {
	...
	public void setWheelNum(String province,String city){
		List<String> listProvince = Arrays.asList(mProvinceDatas);
		if (null == listProvince || listProvince.size() == 0) {
			return;
		}
		int indexProvince = listProvince.indexOf(province);
		if (-1 == indexProvince) {
			indexProvince = 0;
		}
		String[] cities = mCitisDatasMap.get(province);
		List<String> listCity = Arrays.asList(cities);
		if (null == listCity || listCity.size() == 0) {
			return;
		}
		int indexCity = listCity.indexOf(city);
		if (-1 == indexCity) {
			indexCity = 0;
		}
...
//		Toast.makeText(mActivity, "当前选中:"+mCurrentProviceName+","+mCurrentCityName+","
//				+mCurrentDistrictName+","+mCurrentZipCode, Toast.LENGTH_SHORT).show();
		String result = mCurrentProviceName+" "+mCurrentCityName;
		((WheelViewDialog)mDialog).mListener.onProvinceCityClick(mCurrentProviceName, mCurrentCityName);
		if (null != mDialog) {
			mDialog.dismiss();
		}
	}
